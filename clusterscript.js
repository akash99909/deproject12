...
let existingdatasize=1864;
let existingdatasizeper=186.4;
...
function calexistingdatasize(){
    valteds=(existingdatasizeper+existingdatasize)/1024;   
    return valteds;
}
...
function caladitionaldata1(val4){
     val8=overallaverageestimated*val4;
     return val8;
}

function caladitionaldata2(val4,val8){
    val10=(val4+val8)*overallaverageestimated;
    return val10;
}
function caladitionaldata3(val4,val8,val10){
    val12=(val4+val8+val10)*overallaverageestimated;
    return val12;
}
function caladitionaldata4(val4,val8,val10,val12){
    val14=(val4+val8+val10+val12)*overallaverageestimated;
    return val14;
}
...
let overallaverageestimated=0.34;//coming from sheet1
let inputarray=document.querySelectorAll(".inputclass");
console.log(inputarray);
for(let i=0;i<inputarray.length;i++){
    inputarray[i].addEventListener("keydown",function(e){
        if(e.key="Enter" && repfac.value && intdata.value && av_disk.value && diskspaceutil.value && Compressionratio.value ){
           
            val1=calexistingdatasize();
            tot_data.value=Math.round(val1);
